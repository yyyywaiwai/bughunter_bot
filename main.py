...
CLEANUP_TAG_NAMES = {"実装完了", "見送り", "修正完了", "仕様"}
...
class BughunterBot(discord.Client):
    def __init__(self, config: Config) -> None:
        ...
        self._claude_semaphore = asyncio.Semaphore(4)
    ...
    async def on_thread_update(self, before: discord.Thread, after: discord.Thread) -> None:
        if not isinstance(after.parent, discord.ForumChannel):
            return
        before_tags = {tag.name for tag in getattr(before, "applied_tags", [])}
        after_tags = {tag.name for tag in getattr(after, "applied_tags", [])}
        if not (after_tags & CLEANUP_TAG_NAMES):
            return
        if (after_tags & CLEANUP_TAG_NAMES) <= (before_tags & CLEANUP_TAG_NAMES):
            return
        job = get_job_by_thread(self.db_path, after.id)
        if job:
    ...
    def _is_safe_worktree_path(self, worktree_path: Path) -> bool:
        try:
            worktree_path.resolve().relative_to(self.config.worktree_root.resolve())
            return True
        except ValueError:
            return False

    async def _cleanup_worktree_for_thread(self, thread: discord.Thread) -> None:
        job = get_job_by_thread(self.db_path, thread.id)
        if not job or not job.worktree_path:
            return
        if job.id in self._tasks:
            logging.info("Skip worktree cleanup for running job %s", job.id)
            return
        ...
        if not repo_path.exists():
            ...
            if mapped_path:
                repo_path = mapped_path.resolve()
        if not self._is_safe_repo_path(repo_path):
            logging.warning("Skip worktree cleanup: unsafe repo path %s", repo_path)
            return
        if not repo_path.exists():
            logging.warning("Skip worktree cleanup: repo missing %s", repo_path)
            return
